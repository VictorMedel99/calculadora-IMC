export function EstadoMujer(IMC: number) {
  let rest = ''
  if (IMC<18.5) {
      rest = `peso bajo` 
  } else {
...
export function IMCMujer(peso: number,estatura: number){
  let IMC = peso/(Math.pow(estatura,2));
  return  parseFloat(IMC.toFixed(2));
}

export function pesoMinMujer(edad: number,estatura: number){
  let rest: number
  if (edad>=10 && edad<=14) {
...
export function pesoMaxMujer(edad: number,estatura: number){
  let rest: number
  if (edad>=10 && edad<=14) {
